...
//2) -> 2d matrix 대신에 2d vec 쓸 수 있다.
//3) -> 인접 리스트 써서 장단을 비교.
//4) -> 메모리와 시간이 절반 가까이 절약된다. 문제 조건에 따라 인접 리스트를 Sorting 해줘야 한다.
...
int main() {
	...
	for (int i = 1; i <= v; i++) {
		sort(adj_l[i].begin(), adj_l[i].end()); //sort를 해주는 이유는 정점 번호가 낮은 것 부터 방문 하겠다는 문제의 조건 때문이지
	}                                           //인접 리스트의 필수 조건이 아니다. 
	dfs(s);
	memset(visited, false, sizeof(visited));
	cout << endl;
