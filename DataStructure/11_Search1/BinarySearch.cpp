...
int isBSTUtil(node* node, int min, int _max);

/* Returns true if the given tree is a binary search tree (efficient version). */
int isBST(node* node)
{
	return(isBSTUtil(node, INT_MIN, INT__max));
}

/* Returns true if the given tree is a BST and its values are >= min and <= _max. */
int isBSTUtil(node* node, int min, int _max)
{
	...
	/* false if this node violates the min/_max constraint */
	if (node->data < min || node->data > _max)
		return 0;

	/* otherwise check the subtrees recursively, tightening the min or _max constraint */
	return
		isBSTUtil(node->left, min, node->data - 1) && // Allow only distinct values  
		isBSTUtil(node->right, node->data + 1, _max); // Allow only distinct values  
}
